...
- file đề DOCX/PDF gốc được lưu ở thư mục `exam-imports/` trong bucket;
...
- Đề DOCX/PDF tối đa 25 MB; học liệu tối đa 100 MB.
- URL upload ký tạm thời hết hạn sau 10 phút.
- Chỉ tài khoản vai trò `mod` hoặc `admin` được xin URL upload và nhập đề.
- Định dạng học liệu upload trực tiếp: PDF, DOCX, PPTX, MP4, WebM, PNG, JPG, WebP và SVG.

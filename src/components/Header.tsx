...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-sm border-b border-blue-100 sticky top-0 z-50 h-16 flex items-center shrink-0">
      <div className="w-full px-4 sm:px-6 flex items-center justify-between gap-4">
        {/* Branding */}
        <Link href="/" className="flex items-center gap-3 shrink-0 min-w-0">
          <span className="bg-blue-600 text-white w-9 h-9 rounded-xl flex items-center justify-center text-xl font-extrabold shadow-sm shrink-0">
            ∑
          </span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {loading ? (
            ...
            <>
              ...
              <button
                onClick={logout}
                className="text-sm font-medium text-gray-500 hover:text-red-500 border border-gray-200 hover:border-red-300 py-1.5 px-3 rounded-full transition-all"
              >
                Đăng xuất
              </button>
            </>

    ...
    <div className="max-w-7xl mx-auto px-4 py-12 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition-all hover:-translate-y-0.5 text-sm"
          >
            ＋ Tạo đề mới
          </Link>
        </div>
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full max-w-md p-3 border-2 border-gray-200 rounded-xl text-sm focus:border-brand-400 focus:ring-4 focus:ring-brand-500/10 outline-none transition-all"
        />
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          {!loading && (
            <button onClick={fetchExams} className="text-xs text-gray-400 hover:text-brand-600 transition-colors">
              ↻ Làm mới
            </button>
          )}
        </div>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {exam.isShared && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-brand-100 text-brand-600 font-medium">chia sẻ</span>
                        )}
                        {exam.isClonedFromShared && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-brand-100 text-brand-600 font-medium">bản sao</span>
                        )}
                      </div>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-col gap-0.5">
                        {exam.gradeLevel && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-brand-50 text-brand-700 font-semibold w-fit">{exam.gradeLevel}</span>
                        )}
                        {exam.examType && (
                    ...
                    <td className="px-3 py-4 text-center">
                      <span className="bg-brand-100 text-brand-700 font-bold px-2 py-0.5 rounded-full text-xs">{exam.part1Count}</span>
                    </td>
                    <td className="px-3 py-4 text-center">
                      <span className="bg-amber-100 text-amber-700 font-bold px-2 py-0.5 rounded-full text-xs">{exam.part2Count}</span>
                    </td>
                    <td className="px-3 py-4 text-center">
                      <span className="bg-success-100 text-success-700 font-bold px-2 py-0.5 rounded-full text-xs">{exam.part3Count}</span>
                    </td>
                    <td className="px-3 py-4 text-center">
                      <span className="bg-brand-100 text-brand-700 font-bold px-2 py-0.5 rounded-full text-xs">{exam.submissionCount}</span>
                    </td>
                    <td className="px-3 py-4 text-center">
                      <span className={`font-bold px-2 py-0.5 rounded-full text-xs ${
                        exam.maxRetries === 0 ? "bg-gray-100 text-gray-500" : "bg-danger-100 text-danger-700"
                      }`}>

...
const EXAM_EXTENSIONS = new Set(["docx", "pdf"]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxSize = folder === "exam-imports" ? 25 * 1024 * 1024 : 100 * 1024 * 1024;
    const allowed = folder === "exam-imports" ? EXAM_EXTENSIONS : LEARNING_EXTENSIONS;
    ...
    if (!status.configured || (folder === "learning-materials" && !status.publicAccessConfigured)) {
